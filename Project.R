...
for (order in arima_orders) {
  for (season in season_orders){
    ...
    cat("ARIMA(", paste(order, collapse = ","), paste(season, collapse = ","), "): AIC =", arima_model$aicc, "\n")
  }
}
fit_all_arima <- arima(ts_train_all,order=c(2, 1, 0), seasonal = list(order=c(0,1,1),12))
fit_all_arima
...
acf <- acf(fit_all_arima$residuals)
...
theoretical_quantiles <- qnorm(ppoints(length(fit_all_arima$residuals)))
sample_quantiles <- sort(fit_all_arima$residuals)
qqplot_ggplot <- ggplot(fit_all_arima$residuals, aes(x = theoretical_quantiles, y = sample_quantiles)) +
  geom_point() +
  geom_abline(slope = sd(fit_all_arima$residuals), intercept = mean(fit_all_arima$residuals), color = "red", linetype = "dashed") +
  labs(title = "QQ Plot",
       x = "Theoretical Quantiles",
       y = "Sample Quantiles") +
  theme_minimal()

acf_data <- data.frame(Lag = acf$lag[-1],
                        ACF = acf$acf[-1])

acf_ggplot <- ggplot(acf_data, aes(x = Lag, y = ACF)) +
  geom_bar(stat = "identity", width = 0.01, fill = "steelblue") +
  geom_hline(yintercept = c(1.96 / sqrt(length(fit_all_arima$residuals)), -1.96 / sqrt(length(fit_all_arima$residuals))),
  ...
  theme_minimal()
# Combine plots
grid.arrange(residuals_plot, lb_p_values_plot, qqplot_ggplot, acf_ggplot, nrow = 2, ncol=2)
...
fit_ass_arima <- arima(ts_train_ass,order=c(2, 1, 0), seasonal = list(order=c(0,1,1),12))
fit_ass_arima
...
acf <- acf(fit_ass_arima$residuals)
...
theoretical_quantiles <- qnorm(ppoints(length(fit_ass_arima$residuals)))
sample_quantiles <- sort(fit_all_arima$residuals)
qqplot_ggplot <- ggplot(fit_all_arima$residuals, aes(x = theoretical_quantiles, y = sample_quantiles)) +
  geom_point() +
  geom_abline(slope = sd(fit_all_arima$residuals), intercept = mean(fit_ass_arima$residuals), color = "red", linetype = "dashed") +
  labs(title = "QQ Plot",
       x = "Theoretical Quantiles",
       y = "Sample Quantiles") +
  theme_minimal()
...
acf_ggplot <- ggplot(acf_data, aes(x = Lag, y = ACF)) +
  geom_bar(stat = "identity", width = 0.01, fill = "steelblue") +
  geom_hline(yintercept = c(1.96 / sqrt(length(fit_ass_arima$residuals)), -1.96 / sqrt(length(fit_ass_arima$residuals))),
             linetype = "dashed", color = "red", size = 1) +
  labs(title = "ACF of Residuals",
       x = "Lag",
       y = "ACF") +
...
## future forecast using HW model
full_data_tmp <- rbind(data_2015,data_2016,data_2017,data_2018,data_2019,
                       data_2020,data_2021,data_2022,data_2023)

full_data_tmp$timestamp <- ymd_hms(full_data_tmp$OCCURRED_ON_DATE)
full_data_tmp$date <- as.Date(full_data_tmp$timestamp)
full_data_tmp$year_month <- paste(year(full_data_tmp$date), 
                                   sprintf("%02d", month(full_data_tmp$date)), sep = "-")

full_data_all <- full_data_tmp %>%
  group_by(year_month) %>%
  summarise(count = n())
full_data_all <- full_data_all[-1, ]
full_data_all <- full_data_all[-nrow(full_data_all), ]

ts_full <- ts(full_data_all$count, 
                   start = c(2015, 7),
                   frequency = 12)
fit_full_hw=HoltWinters(ts_full,seasonal="additive", gamma=T)
forecast_full_hw <- forecast(fit_full_hw, h=21)
plot(forecast(forecast_full_hw))

full_data_ass_tmp <- full_data_tmp[grepl("^ASSAULT", as.character(data_train_tmp$OFFENSE_DESCRIPTION)), ]

full_data_ass <- full_data_ass_tmp %>%
  group_by(year_month) %>%
  summarise(count = n())
full_data_ass <- full_data_ass[-1, ]
full_data_ass <- full_data_ass[-nrow(full_data_ass), ]

# change the type to TS
ts_full_ass <- ts(full_data_ass$count, 
                   start = c(2015, 7),
                   frequency = 12)
